...
local function highlight_code(code, lang)
  ...
  local patterns = {
    lua = {
      ...
      { pattern = "\\b(local|function|if|then|else|end|for|while|do|return|require|true|false|nil)\\b", class = "keyword" },
    ...
    typescript = {
      ...
      { pattern = "(['\"`][^'\"]*['\"`])", class = "string" },
      { pattern = "(%d+%.?%d*)", class = "number" },
      { pattern = "\\b(interface|class|function|const|let|var|if|else|return|import|export|async|await|try|catch|throw)\\b", class = "keyword" },
    ...
    python = {
      ...
      { pattern = "(['\"][^'\"]*['\"])", class = "string" },
      { pattern = "(%d+%.?%d*)", class = "number" },
      { pattern = "\\b(def|class|if|else|elif|return|import|from|try|except|with|as|True|False|None)\\b", class = "keyword" },
...
function M.generate_preview()
  ...
  for name, color in pairs(colors) do
    color_palette_html = color_palette_html .. 
      string.format('<div><span class="color-swatch" style="background-color: %s;"></span><strong>%s:</strong> <code>%s</code></div>', 
        color, name:gsub("_", " "):gsub("^%l", string.upper), color)
  end
  ...
  for lang, code in pairs(code_samples) do
    ...
    code_samples_html = code_samples_html .. 
      string.format('<div class="code-block"><div class="code-header">%s</div><pre>%s</pre></div>', 
        lang, table.concat(numbered_lines, '\n'))
  end
...
</html>]], css, color_palette_html, code_samples_html)
